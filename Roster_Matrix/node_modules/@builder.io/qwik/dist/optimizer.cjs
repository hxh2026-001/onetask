/**
 * @license
 * @builder.io/qwik/optimizer 1.20.0
    ...
    const resolveId = async (ctx, id2, importerId, resolveOpts) => {
      ...
      if (editMatch) {
        if (!doNotEdit) {
          ...
          if (resolved) {
            const file = null == (_a = devServer.moduleGraph.getModuleById(resolved.id)) ? void 0 : _a.file;
            if (file) {
      ...
      if (parentIds.get(pathId)) {
        ...
        const qrlMatch = null == (_b = /^(?<parent>.*\.[mc]?[jt]sx?)_(?<name>[^/]+)\.js(?<query>$|\?.*$)/.exec(id2)) ? void 0 : _b.groups;
        if (qrlMatch) {
  ...
  async function normalizeRollupOutputOptionsObject(qwikPlugin, rollupOutputOptsObj, useAssetsDir) {
    ...
    if ("client" === opts.target) {
      ...
      fileName = "production" !== opts.buildMode || opts.debug ? chunkInfo => {
        var _a, _b;
        if (null == (_a = chunkInfo.moduleIds) ? void 0 : _a.some(id => id.endsWith("core.prod.mjs") || id.endsWith("core.min.mjs"))) {
