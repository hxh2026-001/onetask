...
// packages/qwik-city/src/middleware/shared/url.ts
var LEADING_DOUBLE_SLASH_REG = /^[\\/]{2,}/;
function normalizeRequestUrl(url, base) {
  const DOUBLE_SLASH_REG = /\/\/|\\\\/g;
  const queryOrHashIndex = url.search(/[?#]/);
  const path3 = queryOrHashIndex === -1 ? url : url.slice(0, queryOrHashIndex);
  const suffix = queryOrHashIndex === -1 ? "" : url.slice(queryOrHashIndex);
  return new URL(
    `${path3.replace(LEADING_DOUBLE_SLASH_REG, "/").replace(DOUBLE_SLASH_REG, "/")}${suffix}`,
    base
  );
}
...
function runQwikCity(serverRequestEv, loadedRoute, requestHandlers, rebuildRouteInfo, trailingSlash = true, basePathname = "/", qwikSerializer) {
  ...
  try {
    ...
    return {
      ...
      requestEv: { headersSent: false },
      completion: Promise.resolve(ensureError(err))
    };
  }
  const completion = (asyncStore ? asyncStore.run(requestEv, runNext, requestEv, rebuildRouteInfo, resolve4) : runNext(requestEv, rebuildRouteInfo, resolve4)).catch(ensureError);
  return {
    response: responsePromise,
    requestEv,
    completion
